...
print("connect via", VUER_DEV_URI + "?ws=wss://" + VUER_DEV_URI.split("//")[1] + "")
...
async def main(sess: VuerSession):
    ...
    sess.upsert @ Movable(key='movable-left', position=[0.15, 1.1, -0.3], localRotation=True)
    sess.upsert @ Movable(key='movable-right', position=[-0.15, 1.1, -0.3], localRotation=True)
